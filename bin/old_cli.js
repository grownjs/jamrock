...
module.exports = input => {
  ...
  ctx.main = () => {
    ...
    argv.flags.resolvers = argv.flags.resolvers || 'resolvers';
    argv.flags.handlers = argv.flags.handlers || 'handlers';
    argv.flags.routes = argv.flags.routes || 'routes';

    if (!argv.flags.app && argv.flags.cwd) {
      argv.flags.app = argv.flags.cwd.replace(/[^/]+(?=\/)/g, '..').replace(/[^/]+$/, 'server');
    } else {
      argv.flags.app = argv.flags.app || 'server';
    }

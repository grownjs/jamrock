export function run(test, driver, expect, createSandbox) {
  test.group('ssr/dom testing support', ({ Event }) => {
    ...
    test(`${driver}: ensure client-side components can be rendered`, async () => {
      const mod = env.lookup('generated/components/main.html');
      ...
      expect([...el.querySelectorAll('button')].map(x => x.outerHTML)).toEqual([
        '<button data-location="generated/components/main.html:43:3" class="jam-x1kixmrr">insight</button>',
        '<button data-location="generated/components/main.html:44:3" class="jam-x1kixmrr">truth</button>',
      ]);

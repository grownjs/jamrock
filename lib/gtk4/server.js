...
export function createSoupServer(location, callback, clients, events) {
  ...
  function websocketHandler(_server, _msg, _path, connection) {
    printLog(`[WS] connected, connection type: ${connection?.constructor?.name}`);
...
export async function createServer(env, options) {
  ...
  const server = createSoupServer(location, (req, _server) => {
    ...
    return handler.call(req, () => clients, () => _server.stop());
  }, clients, events);

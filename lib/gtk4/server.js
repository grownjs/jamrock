...
function createSoupServer(location, callback) {
  function _handle(_server, msg, _path, _query, next) {
    console.log(1);
    const params = {
  ...
  function handler(_server, msg, _path, _query) {
    ...
    let value = -1;
    new Promise(ok => {
      _handle(_server, msg, _path, _query, (x) => {
        console.log('NEXT', x);
        resp = x;
      });
      setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      msg.get_response_body().append(`
                <html>
                <body>${msg.get_remote_host()}: ${value}</body>
                </html>
            `);
    }, 3000);
  }
...
export async function createServer(env, options) {
  ...
  const server = createSoupServer(location, (req, _server) => {
    return resolve(req) || handler.call(req, () => clients, () => _server.stop()).then(finalResponse);
  });

...
export async function getSessionCookies(req, store) {
...
export async function createHandler(env, options) {
  ...
  const { store, location } = await getLocationStore(options);
  ...
  const call = (req, teardown) => {
    ...
    const { session, cookies } = await getSessionCookies(req, store);

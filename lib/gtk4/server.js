...
import { path, serverResponse, timingSafeEqual, serveStaticFiles, GLib, Soup, Gio } from './deps.js';

import { Handler } from '../../dist/main.mjs';
import { createStore, createConnection, createResponseSync, parseLocation, finalResponse, serveFrom } from '../../dist/server.mjs';
  ...
  const call = (req, clients, teardown) => {
    ...
    if (!resp) {
      const conn = {
        ...
        method: req.method,
        headers: req.headers,
        request_path: GLib.Uri.parse(req.url, GLib.UriFlags.NONE).get_path(),
      };

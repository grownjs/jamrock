import { fs, url, path, loadEnv, getCwd, globSync, getVersion, exitProgram, GLib, Soup } from './deps.js';
...
async function fetch(_url, opts = {}) {
  ...
  const msg = new Soup.Message(method, _url);
...
const crypto = {
  subtle: {
    ...
    async sign(algorithm, key, data) {
      ...
      // compute_hmac_for_string requires 4 args: (digestType, key, str, length=-1)
      const hexResult = GLib.compute_hmac_for_string(GLib.ChecksumType.SHA1, keyStr, dataStr, -1);
      // Convert hex string to ArrayBuffer so new Uint8Array(sig) works in the caller
...
// btoa/atob are Web APIs not available in GJS — provide GLib-based implementations
function btoa(str) {
...
Object.assign(globalThis, { process, fetch, Buffer, Headers, Response, Request, URLSearchParams, XMLHttpRequest, crypto, btoa, atob });

...
// Node.js module polyfills - MUST be registered before importing main.mjs
const nodeUtil = {
...
// Register modules BEFORE importing main.mjs
registerModule('util', nodeUtil);
registerModule('url', nodeUrl);
registerModule('path', nodePath);
registerModule('fs', nodeFs);

// NOW import gtk-main.mjs - it will find the registered modules
import { Util, Template } from '../../dist/gtk-main.mjs';
import { fetch } from '../../dist/gtk.mjs';

// Update nodePath with Template methods (now that Template is loaded)
nodePath.join = Template.join;
nodePath.dirname = Template.dirname;

// Update path exports with Template methods
...
Template.exists = x => fs.existsSync(x);

// GJS transpiler: handles Less CSS compilation only; esbuild (JS bundling) is not
// available in GJS so JS bundles are passed through as-is.
Template.transpile = async function transpile(tpl, ext, opts) {
  if (Array.isArray(tpl)) {
    return Promise.all(tpl.map(x => transpile(x, ext, opts)));
  }
  const params = { ...tpl.attributes };
  if (ext === 'css' && tpl.attributes?.lang === 'less' && opts?.use?.less) {
    ...
    return { params, parent: tpl.ref, content: out.css, children: out.imports };
  }
  // JS bundling not available — pass through as-is
  return { params, parent: tpl.ref, content: tpl.content, children: tpl.children || [] };
};

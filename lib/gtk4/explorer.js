...
import {
  ...
  signal, computed, define, syncToStore,
} from '../../dist/gtk.mjs';
...
// Strip emoji and variation selectors from label text before passing to GTK.
// GTK4/Pango crashes on 4-byte emoji (U+1F000+) and multi-codepoint sequences
// (e.g. U+FE0F variation selector) in Gtk.Label / Gtk.Button text.
// Box-drawing chars (U+2500-U+257F) and BMP symbols are safe and kept.
function sanitizeLabel(text) {
  if (typeof text !== 'string') return text;
  return text
    .replace(/\p{Emoji_Presentation}/gu, '')  // remove emoji (presentation form)
    .replace(/[\u{FE00}-\u{FE0F}]/gu, '')      // remove variation selectors
    .replace(/[\u{1F000}-\u{1FFFF}]/gu, '')    // remove supplementary multilingual plane emoji
...
function vnodeToWidget(vnode, state = {}, subscriptions = []) {
  if (!vnode) return null;
  if (typeof vnode === 'string') return new Gtk.Label({ label: sanitizeLabel(vnode) });
  if (typeof vnode === 'number') return new Gtk.Label({ label: String(vnode) });
  if (!Array.isArray(vnode)) return new Gtk.Label({ label: sanitizeLabel(String(vnode)) });
  ...
  if (name === 'vstack' || name === 'button') {
    // Debug: log vnode structure
  }

  const flatChildren = flattenChildren(children);

  if (name === 'label') {
    const widget = new Gtk.Label({ label: '', wrap: true });
    ...
    const updateLabel = () => {
      const textParts = Array.isArray(flatChildren)
        ...
        : [];
      widget.set_label(sanitizeLabel(textParts.join('')));
    };

    updateLabel();

    const hasSignalFn = Array.isArray(flatChildren) && flatChildren.some(c => typeof c === 'function');
    if (hasSignalFn) {
      for (const sig of Object.values(state)) {
        if (sig && typeof sig.subscribe === 'function') {
          subscriptions.push(sig.subscribe(updateLabel));
        }
      }
    }
  ...
  if (name === 'button') {
    ...
    const widget = factory(sanitizeLabel(buttonText), { onClick: handler });

    if (typeof text === 'function') {
      const updateButton = () => {
        try {
          widget.set_label(sanitizeLabel(String(text() ?? 'Button')));
        } catch {
          // ignore
        }
      };
      for (const sig of Object.values(state)) {
        if (sig && typeof sig.subscribe === 'function') {
          subscriptions.push(sig.subscribe(updateButton));
        }
      }
    }
  ...
  if (name === 'scroll') {
    ...
    const childWidget = child ? vnodeToWidget(child, state, subscriptions) : null;
  ...
  const childWidgets = Array.isArray(flatChildren)
    ? flatChildren.map(c => vnodeToWidget(c, state, subscriptions)).filter(Boolean)
    : [];
...
function createSidebar(componentFiles, compiledComponents, onSelect) {
  const sidebarItems = [
  ...
  for (const file of componentFiles) {
    // Skip components that failed to preload
    if (!compiledComponents.has(file)) {
      continue;
    }
...
export async function createExplorer({ env, options }) {
  ...
  try {
    ...
    console.log('>>> Compiler reload skipped:', e.message);
  }
  ...
  async function preloadComponents() {
    ...
    for (const filename of componentFiles) {
      try {
        ...
        if (!mod.__vdom && !mod.__gtk) continue;
        ...
        if (mod.__gtk) {
          // DSL path - direct widget creation
          useDSL = true;
          result = { body: [null] }; // Placeholder
        } else {
          // Vnode path
          result = await Template.resolve(mod, filepath, ctx, data, null);
        }

        compiledComponents.set(filename, { result, data, mod, filepath, useDSL });
        console.log('>>> Preloaded:', filename, useDSL ? '(DSL)' : '(vnode)');
      } catch (e) {
  ...
  createApplication({
    ...
    function loadAndRender(filename) {
      if (isLoading) {
        console.log('>>> Skipping (already loading):', filename);
        return;
      }
      isLoading = true;
      ...
      try {
        ...
        const { result, data, mod, useDSL } = compiled;
        currentState = data;
        ...
        if (useDSL && mod.__gtk) {
          // DSL path - direct widget creation
          console.log('>>> Using DSL path');
          const gtkResult = mod.__gtk(GTK_ELEMENTS, currentState);
          widget = gtkResult.widget;
          // DSL handles subscriptions internally
          console.log('>>> DSL widget created:', widget?.constructor?.name);
        } else {
          // Vnode path
          widget = result && result.body && result.body[0]
            ? vnodeToWidget(result.body[0], currentState, subscriptions)
            : new Gtk.Label({ label: 'Empty component' });
          console.log('>>> Vnode widget created:', widget?.constructor?.name);
    ...
    open(() => {
      const sidebar = createSidebar(componentFiles, compiledComponents, filename => {
        console.log('\n>>> CLICKED:', filename);
        loadAndRender(filename);
      });

...
function vnodeToWidget(vnode, state = {}) {
  ...
  if (name === 'spinbutton' || name === 'spinner' || name === 'calendar' || name === 'clock' || name === 'separator' || name === 'image' || name === 'progress' || name === 'level') {
    return factory({});
  }
...
export async function createExplorer({ env, options }) {
  ...
  async function loadComponent(filename) {
    console.log('>>> loadComponent called with:', filename);
    ...
    console.log('>>> Module located, has template:', !!mod.__template);
  ...
  createApplication({
    ...
    async function loadAndRender(filename) {
      ...
      if (!info) {
        console.log('Component not found:', filename);
      ...
      console.log('Body:', result?.body ? result.body.length : 'none');
      ...
      console.log('Widget:', widget ? widget.constructor.name : 'null');
    ...
    open(() => {
      const sidebar = createSidebar(componentFiles, filename => {
        console.log('\n>>> CLICKED:', filename);
        console.log('>>> EXPECT: Component should load in right pane...\n');

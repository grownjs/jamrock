...
function vnodeToWidget(vnode, state = {}, subscriptions = []) {
  ...
  if (name === 'label') {
    ...
    const updateLabel = () => {
      const textParts = Array.isArray(flatChildren)
        ...
        : [];
      widget.set_label(textParts.join(''));
    };
  ...
  if (name === 'button') {
    ...
    const widget = factory(buttonText, { onClick: handler });
    
    if (typeof text === 'function') {
      const updateButton = () => {
        try {
          widget.set_label(String(text() ?? 'Button'));
        } catch {
          // ignore
        }
      };
...
function createSidebar(componentFiles, compiledComponents, onSelect) {
  ...
  // Complex components that cause crashes (GTK memory issue)
  const complexComponents = ['gallery.html', 'calculator.html', 'form.html', 'settings.html', 'pomodoro.html'];
  ...
  for (const file of componentFiles) {
    ...
    // Skip complex components that crash
    if (complexComponents.some(c => file.includes(c))) {
      continue;
    }
...
export async function createExplorer({ env, options }) {
  ...
  async function preloadComponents() {
    ...
    for (const filename of componentFiles) {
      try {
        ...
        if (!mod.__template && !mod.__gtk) continue;
  ...
  createApplication({
    ...
    open(() => {
      ...
      setupHotReload();
      // Load button.html (index 1) instead of gallery.html (index 0)
      loadAndRender(componentFiles[1] || componentFiles[0]);

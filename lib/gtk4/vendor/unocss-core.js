...
async function resolveConfig(userConfig = {}, defaults = {}) {
  ...
  for (const [index, rule] of rules.entries()) {
    ...
    if (isStaticRule(rule))
      toArray(meta.prefix ?? "").forEach((prefix) => {
...
var UnoGeneratorInternal = class UnoGeneratorInternal2 {
  ...
  async stringifyShortcuts(parent, context, expanded, meta = { layer: this.config.shortcutsLayer }) {
    ...
    for (const item of parsed) {
      ...
      for (const { selector, entries, parent: parent$1, sort, noMerge, layer } of this.applyVariants(item, variants, raw))
        layerMap.getFallback(layer ?? meta.layer, new TwoKeyMap()).getFallback(selector, parent$1, [[], item[0]])[0].push([
          entries,

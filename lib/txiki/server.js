...
async function handleConn(conn, env, store, location, options) {
  const buffer = [];
  ...
  const connObj = createConnection(store, session, cookies, options, req, location, () => {});
  const resp = await createResponse(env, connObj, () => [], options);
...
export async function createServer(env, options) {
  ...
  for await (const conn of server) {
    handleConn(conn, env, store, location, options);
  }
}

/* global Deno */

import './runtime.js';

import { path, editor, staticFiles, createHash, timingSafeEqual } from './deps.js';

import { Util } from '../../dist/main.mjs';
import {
  createStore, createSession, createConnection, extendConnection, createRedisConnection, createResponse, parseLocation, parseCookies, finalResponse, serveFrom,
} from '../../dist/server.mjs';

const DIST_DIR = path.resolve(import.meta.url.replace('file://', ''), '../../../dist');

export async function createHandler(env, options) {
  const serveFile = staticFiles(options.public || 'public');
  const location = parseLocation(options);
  const store = await createStore({
    encode: (v, s) => createHash('sha1').update(s + v, 'ascii').digest('base64'),
    compare: (a, b) => a && b && timingSafeEqual(Buffer.from(a), Buffer.from(b)),
  }, options);

  let sync;
  if (options.watch) {
    sync = env.watcher;
  }

  const call = async (request, teardown) => {
    let resp = await serveFile({ request, respondWith: r => r });
    if (!resp.ok) {
      const cookies = parseCookies(request.headers.get('cookie') || '');
      const session = await createSession(store, cookies.sid || '$');
      const server = { protocol: location.protocol, hostname: location.hostname, port: location.port, teardown };
      const conn = extendConnection(createConnection(request, server, options), { store, session, cookies });

      resp = await createResponse(env, conn, options);
      await store.write(conn.req.sid, conn.session);
    }
    return resp;
  };

  return {
    handler: { call, sync },
    location,
  };
}

export async function createServer(env, options) {
  await createRedisConnection(env, options, () => import('npm:redis'));
  const { handler, location } = await createHandler(env, options);

  const resolve = serveFrom(env, DIST_DIR, editor);
  const server = options.https
    ? Deno.listenTls({
      ...location,
      key: Deno.readTextFileSync(process.env.SSL_KEY_FILE),
      cert: Deno.readTextFileSync(process.env.SSL_CRT_FILE),
    })
    : Deno.listen(location);

  if (!options.quiet) {
    console.debug(`Listening on ${location.protocol}://${location.hostname}:${location.port}/`);
  }

  async function handleReq(req) {
    if (handler.sync) await handler.sync.rebuild(req);
    return resolve(req) || finalResponse(await handler.call(req, () => server.close()));
  }

  async function serveHttp(conn) {
    const httpConn = Deno.serveHttp(conn);

    try {
      for await (const requestEvent of httpConn) {
        const response = await handleReq(requestEvent.request);

        requestEvent.respondWith(response).catch(e => {
          if (!e.message.includes('connection closed') && e.name !== 'BadResource') Util.trace(e, 'E_HTTP');
        });
      }
    } catch (e) {
      Util.trace(e, 'E_HTTP');
    }
  }

  (async () => {
    for await (const conn of server) serveHttp(conn);
  })();
}

...
var EventHub = class {
  ...
  onSubmit() {
    return (e) => {
      if ("confirm" in e.target.dataset || "async" in e.target.dataset || "trigger" in e.target.dataset) {
        e.preventDefault();
        this.require(0, e.target, import("./submit-4BTAYSMT.js")).then(({ handleSubmit }) => handleSubmit.call(this, e));
      }
    };
  }
  onHandle(kind) {
    return (e) => {
      ...
      return this.confirm(e.target, () => {
        return this.require(1, e.target, import("./handler-WONDDUMG.js")).then(({ handleEvent }) => handleEvent.call(this, e, kind));
      });
    };
  }
  async loadURL(el, ...args) {
    return this.confirm(el, async () => {
      ...
      this.browser.pause();
      try {
        const { loadPage } = await import("./request-WIGTME3C.js");
  ...
  constructor(state, prefix, version) {
    ...
    this.runtime = async () => {
      if (!window.Jamrock.Runtime) {
        const { createRender, createFragment } = await import("./elements-WN5FP5YP.js");

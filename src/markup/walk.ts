...
import { Is, repeat, identifier } from '../utils/server.ts';
...
export function traverse(obj: any[], html: string, parent: any, context: any, counter: number = 0): any[] {
  const preserve = context.file?.includes('+page');
  const stack: any[] = [];
  const copy: any[] = [];

  let inSnippet: boolean | undefined;
  let chunk: any[] = [];
  obj.forEach((node: any) => {
    ...
    if (node.type === 'element') {
      ...
      if (!NOT_ANCHORS.includes(node.rawTagName) && node.position) {
        if (!(node.rawTagName === 'input' && node.attributes.some((x: any) => x.key === 'hidden' || (x.key === 'type' && x.value === 'hidden')))) {
      ...
      if (node.rawTagName === 'script' || node.rawTagName === 'style') {
        ...
        const baseNode: any = {
          ...
          attributes: node.attributes
            ? node.attributes.reduce((memo: Record<string, any>, { key, value }: { key: string; value: any }) => {
      ...
      if (context.response?.rules) {
        node.attributes.forEach(({ key, value }: { key: string; value: any }) => {
          let newClass;
      ...
      if (context.stack && node.content.includes('\0')) {
        copy.push(...node.content.split('\0')
          .reduce((memo: any[], _: any, i: number, c: any[]) => memo

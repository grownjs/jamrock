import { blocks, vars } from 'eslint-plugin-jamrock/util.js';
import { RE_MATCH_ROUTES } from 'eslint-plugin-jamrock/const.js';

import { Expr } from './expr.ts';
import { render } from './mkd.ts';
import { traverse } from './walk.ts';
import { lexer } from '../templ/utils.ts';
import { reduce, visit } from './utils.ts';
import { reduceGTK, getSubscriptions, resetGTKCompiler } from './gtk-utils.ts';
import { Template } from '../templ/main.ts';
import { extract, rebase } from '../handler/utils.ts';
import { Is, parseMarkup, identifier, ignore, dump } from '../utils/server.ts';

const RE_EXPORT_DEFAULT = /\nexport default[\s{]/;
const RE_RESOLVE_IMPORTS = /\/\*@@\*\/__resolve\('(.+?)'\)/g;
const RE_MATCH_IMPORTS = /\bimport([^;]+?)from\s*(['""])(.+?)\2[\n;]?/g;

interface BlockAssets {
  js: any[];
  css: any[];
  media: string[];
}

export class Block {
  assets: BlockAssets = { js: [], css: [], media: [] };

  declare readonly id: string;
  declare readonly src: string;
  declare readonly dest: string;
  declare readonly base: string;
  declare readonly code: string;
  declare readonly opts: any;
  declare readonly doc: any;
  declare readonly meta: any[];
  declare readonly attrs: any;
  declare readonly chunks: any[];
  declare readonly locations: any[];
  declare readonly children: any[];
  declare readonly imports: any[];
  declare readonly calls: Set<string>;

  context?: string;
  markup: any;
  fragments: any;
  snippets: any;
  scripts: any[];
  styles: any[];
  rules?: string[];
  media?: string[];
  declare module: any;
  declare script: any;

  constructor(tpl: string, file: string, options: any) {
    const opts = { ...options };
    const base = `${opts.cwd || '.'}/`;
    const src = rebase(file.replace(base, ''));
    const dest = rebase(`${base}${src}`);
    const id = opts.scope || (identifier('jam', src) as string[]).join('-');

    const __dirname = Template.dirname(`${base}/${src}`);
    const chunks: any[] = [];

    tpl = tpl.replace(/```(\w+\n)?([^]*?)```/g, (_: string, $1: string, $2: string) => {
      chunks.push({ code: $2 });
      return ['```', $1 || '', $2.replace(/\S/g, ' '), '```'].join('');
    });

    // Strip inline code spans containing HTML tags before the HTML parser runs.
    // Content is blanked (non-whitespace → space) to keep line/col identity intact,
    // so that tags like `<style lang="less">` are not mistaken for real elements.
    tpl = tpl.replace(/`([^`\n]+)`/g, (_: string, $1: string) => {
      if (!$1.includes('<')) return _;
      chunks.push({ code: $1, inline: true });
      return `\`${$1.replace(/\S/g, ' ')}\``;
    });

    Object.defineProperty(this, 'id', { value: id });
    Object.defineProperty(this, 'src', { value: src });
    Object.defineProperty(this, 'dest', { value: dest });
    Object.defineProperty(this, 'base', { value: __dirname });
    Object.defineProperty(this, 'code', { value: tpl });
    Object.defineProperty(this, 'opts', { value: opts });
    Object.defineProperty(this, 'doc', { value: {} });
    Object.defineProperty(this, 'meta', { value: [] });
    Object.defineProperty(this, 'attrs', { value: {} });
    Object.defineProperty(this, 'chunks', { value: chunks });
    Object.defineProperty(this, 'assets', { enumerable: false });

    const { locations } = blocks(this.code, false);

    const locate = (offset: number, value: any) => {
      let found;
      for (const chunk of locations) {
        found = chunk;
        if (chunk.block === value && chunk.offset[0] >= offset) break;
      }
      return found;
    };

    const metadata: any = {
      response: {
        fragments: {},
        snippets: {},
        scripts: [],
        styles: [],
        markup: {},
        rules: [],
        media: [],
      },
      locate,
      lexer,
      file: this.src,
    };

    const tree = parseMarkup(this.code, { includePositions: true }) as any[];

    metadata.response.markup.content = traverse(tree, this.code, null, metadata);

    Object.assign(this, metadata.response);

    if (this.markup.attributes) {
      Object.assign(this.attrs, this.markup.attributes);
      delete this.attrs['@location'];
    }

    if (this.markup.metadata) {
      this.meta.push(...this.markup.metadata);
    }

    if (this.markup.document) {
      Object.assign(this.doc, this.markup.document);
    }

    let imports: any[] = [];
    let children: any[] = [];
    if (!this.code.includes('<script')) {
      this.context = 'static';
    } else {
      const contexts = this.scripts.reduce((memo: string[], cur: any) => memo.concat(cur.attributes.context || []), []);

      this.context = contexts.some((_: string) => _ === 'client') ? 'client' : 'module';

      Object.defineProperty(this, 'module', {
        value: vars(this.scripts
          .filter((x: any) => !x.root && !x.attributes.scoped && x.attributes.context === 'module')
          .map((x: any) => x.content).join('\n')),
      });

      Object.defineProperty(this, 'script', {
        value: vars(this.scripts
          .filter((x: any) => !x.root && !x.attributes.scoped && !x.attributes.global && x.attributes.context !== 'module')
          .map((x: any) => x.content).join('\n')),
      });

      this.scripts.forEach((_: any) => {
        if (_.attributes.scoped) {
          const { prelude, interlude } = Block.script(_.content);

          _.content = `${Block.unwrap(prelude, file, dest)}\nexport function __execute(self) {${interlude}};\nexport default {__execute};`;
        }
      });

      this.styles.forEach((_: any) => {
        if (_.attributes.src) {
          const ext = _.attributes.src.split('.').pop();

          _.content = `@import "${_.attributes.src}";`;
          if (ext !== 'css') _.attributes.lang = ext;
          delete _.attributes.src;
        }
      });

      lexer(Block.module(this.module.code), { position: { line: 1, col: this.module.code.indexOf('\n') } });
      lexer(Block.module(this.script.code, true), { position: { line: 1, col: this.script.code.indexOf('\n') } });

      children = this.module.children.concat(this.script.children)
        .filter((_: string) => _.includes('.md') || _.includes('.html'))
        .map((_: string) => ({ ref: _, src: Template.join(this.base, _) }))
        .map((_: any) => Object.defineProperty(_, 'code', { get: () => Template.read(_.src) }));

      imports = this.module.children.concat(this.script.children)
        .filter((_: string) => !(_.includes('.md') || _.includes('.html')) && _[0] === '.')
        .map((_: string) => ({ ref: _, src: Template.join(this.base, _) }));
    }

    Object.defineProperty(this, 'locations', { value: locations });
    Object.defineProperty(this, 'children', { value: children });
    Object.defineProperty(this, 'imports', { value: imports });
    Object.defineProperty(this, 'calls', { value: new Set() });
  }

  get $attributes(): string {
    return `($$) => ({${Block.wrap(Expr.props(this.attrs, '\t'))}})`;
  }

  get $metadata(): string {
    return `($$) => [${Block.wrap(reduce(this.meta, this.context, 1))}]`;
  }

  get $doctype(): string {
    return `($$) => ({${Block.wrap(Expr.props(this.doc, '\t'))}})`;
  }

  get $fragments(): string {
    const scope: any = { ...this.module?.locals, ...this.script?.locals };

    return Object.entries(this.fragments)
      .map(([fn, _]: [string, any]) => `\n\t'${fn}': {
    s: ${JSON.stringify(_.locals?.filter((k: string) => scope[k]) || [])},
    a: ($$) => ({${Block.wrap(Expr.props(_.attributes, '\t'))}}),
    r: ($$) => [${Block.wrap(reduce(_.elements, this.context, 1))}] },`)
      .join('');
  }

  get $snippets(): string {
    return Object.entries(this.snippets)
      .map(([fn, _]: [string, any]) => `\n\t${fn}: (${_.args.join(', ')}) => ($$) => [${Block.wrap(reduce(_.body, this.context, 1))}]`)
      .join('');
  }

  get $scripts(): any[] {
    return this.assets.js.map(([ref, id, deps]: any[]) => [ref, this.opts.cwd ? rebase(id, this.opts.cwd) : id, deps]);
  }

  get $styles(): any[] {
    return this.assets.css.map(([id, deps]: any[]) => [this.opts.cwd ? rebase(id, this.opts.cwd) : id, deps]);
  }

  get $assets(): string[] {
    return this.assets.media;
  }

  get $prefix(): string {
    const resources = JSON.stringify(this.$assets);
    const javascript = JSON.stringify(this.$scripts);
    const stylesheets = JSON.stringify(this.$styles);

    return `export const __snippets = {${this.$snippets}};

export const __fragments = {${this.$fragments}};

export const __scripts = ${javascript};
export const __styles = ${stylesheets};
export const __media = ${resources};

export const __context = ${JSON.stringify(this.context)};
export const __doctype = ${this.$doctype};
export const __metadata = ${this.$metadata};
export const __attributes = ${this.$attributes};
`;
  }

  resolve(node: any, resources: any): void {
    const { src, href, inline } = node.attributes;
    const path = src || href;
    const srcAttr = src ? 'src' : 'href';

    if (!path) return;

    if (Is.str(path)) {
      if (path.includes('://')) return;
      if (path.charAt(0) === '/') return;

      const file = Template.join(this.base, path);

      if (!Template.exists(file)) {
        throw new Error(`File not found '${path}' (${this.src})`);
      }

      this.children.push(file);

      if (!resources.media.includes(file)) {
        resources.media.push(file);
      }

      if (node.name === 'svg') {
        const size = node.attributes.size;

        delete node.attributes.size;
        delete node.attributes.href;
        delete node.attributes.src;
        delete node.attributes.inline;

        if (size) {
          node.attributes.width = node.attributes.width || size;
          node.attributes.height = node.attributes.height || size;
        }

        if (inline) {
          const svgContent = Template.read(file)
            .trim()
            .replace(/>\s*</g, '><');

          const innerContent = svgContent
            .replace(/<svg[^>]*>/, '')
            .replace(/<\/svg>$/, '');

          node.attributes['@html'] = innerContent;
        } else if (node.elements.length > 0) {
          node.attributes.xmlns = node.attributes.xmlns || 'http://www.w3.org/2000/svg';
        } else {
          node.elements.push({
            name: 'use',
            type: 'element',
            attributes: {
              'xlink:href': `#${Template.filename(path, '.svg')}`,
              'data-location': file,
            },
          });
        }
      } else {
        if (node.attributes.href) node.attributes.href = `@/${file}`;
        if (node.attributes.src) node.attributes.src = `@/${file}`;
      }
    } else if (path instanceof Expr) {
      const originalName = node.name;
      const originalAttrs = { ...node.attributes };

      node.name = 'resource';
      node.attributes = {
        'data-tag': originalName,
        'data-src': srcAttr,
        [srcAttr]: path,
      };

      if (originalAttrs.inline) {
        node.attributes['data-inline'] = true;
      }
      if (originalAttrs.size) {
        node.attributes.size = originalAttrs.size;
      }
      if (originalAttrs.width) {
        node.attributes.width = originalAttrs.width;
      }
      if (originalAttrs.height) {
        node.attributes.height = originalAttrs.height;
      }
      if (originalAttrs.class) {
        node.attributes.class = originalAttrs.class;
      }
    }
  }

  async markdown(node: any, resources: any): Promise<void> {
    const inline = node.elements.length === 1
      && node.elements[0].inline;

    node.name = node.attributes.tag || (inline ? 'p' : 'template');
    node.elements = await render(node.elements, inline, null, this.opts.markdown);
    delete node.attributes.tag;

    await visit(node.elements, async (_node: any) => {
      if (_node.name === 'img') {
        this.resolve(_node, resources);
      }
      return node;
    });
  }

  async transform(elements: any, resources: any): Promise<void> {
    if (this.src.includes('+page')) {
      this.markup.content = await render(this.markup.content, null, this.chunks, this.opts.markdown);
    }

    await this.traverse();

    await visit(this.markup.metadata, async (node: any) => {
      switch (node.name) {
        case 'link':
          if (node.attributes.rel === 'icon') {
            this.resolve(node, resources);
          }
          if (node.attributes.rel === 'stylesheet' && node.attributes.inline && process.env.NODE_ENV === 'production') {
            node.name = 'style';
            node.attributes = { '@html': await Template.refetch(node.attributes.href, this.base, this.opts) };
          }
          break;

        default:
          if (elements?.[node.name]) {
            const newNode = await elements[node.name](node);
            if (newNode) return newNode;
          }
          break;
      }
      return node;
    }, this.locations);

    await visit(this.markup.content, async (node: any) => {
      Object.keys(node.attributes).forEach((key: string) => {
        if (key.indexOf('@use:') === 0) this.calls.add(key.split(':')[1]);
      });

      switch (node.name) {
        case 'mkd':
          this.markdown(node, resources);
          break;

        case 'source':
        case 'embed':
        case 'track':
        case 'svg':
        case 'img':
          this.resolve(node, resources);
          break;

        default:
          if (elements?.[node.name]) {
            const newNode = await elements[node.name](node);
            if (newNode) return newNode;
          }
          break;
      }
      return node;
    }, this.locations);
  }

  async traverse(): Promise<void> {
    await visit(this.markup.content, (node: any) => {
      if (node.type === 'fragment' && node.offset && this.locations) {
        Object.defineProperty(node, 'locals', {
          enumerable: true,
          writable: false,
          value: [...new Set(this.locations
            .filter((x: any) => x.offset[0] > node.offset.close && x.offset[0] < node.offset.end)
            .reduce((memo: string[], k: any) => memo.concat(k.locals.map((u: any) => u.name)), []))],
        });
      }
    });
  }

  toString(): string {
    const defaults = '__src,__dest,__media,__context,__snippets,__fragments,__scripts,__styles,__doctype,__metadata,__attributes,__vdom';
    
    const isGTK = this.opts.target === 'gtk';
    
    if (isGTK) {
      resetGTKCompiler();
    }
    
    const template = isGTK
      ? reduceGTK(this.markup.content, this.context, 1)
      : reduce(this.markup.content, this.context, 1);

    if (!this.script) {
      const scope = Object.keys(this.snippets)
        .map((_: string) => `${_}=__snippets.${_}`)
        .concat(this.opts.props || [])
        .concat('...$$props').join(',');

      if (isGTK) {
        return `/* eslint-disable */
${this.$prefix}
${this.buildGTKTemplate(template, [])}
export default {${defaults.replace('__vdom', '__gtk')}};
`.replace(/\(\$\$\)/g, '($$$$,$$$$props)');
      }

      return `/* eslint-disable */
${this.$prefix}
export const __vdom = ($$,{${scope}}) => {
  return [${Block.wrap(template)}];
};
export default {${defaults}};
`.replace(/\(\$\$\)/g, '($$$$,$$$$props)');
    }

    const aliases = this.script.aliases;
    const locals = this.script.locals;
    const keys = this.script.keys;

    const functions = this.module.deps.filter((_: string) => this.module.locals[_] === 'function');
    const exported = keys.filter((x: string) => ['let', 'const', 'export'].includes(locals[x])).map((x: string) => aliases[x] || x);
    const calls = [...this.calls].filter((_: string) => functions.includes(_));

    let { prelude, interlude, hasImports } = Block.script(this.script.code);
    if (!interlude && !hasImports) {
      interlude = prelude;
      prelude = '';
    }

    const matched = extract(interlude, true);

    matched.code = Block.exports(matched.code);

    const scope = keys.concat(this.script.deps)
      .reduce((memo: any[], key: string) => {
        if (aliases[key] && locals[key] === 'export') {
          memo.push([aliases[key], key]);
        } else {
          memo.push(key);
        }
        return memo;
      }, []);

    const shared = this.script.imports.jamrock ?? [];
    const lets = scope.map((x: any) => (Is.arr(x) ? x.join(':') : x))
      .filter((local: string) => !shared.includes(local))
      .concat(Object.keys(this.snippets))
      .concat(this.opts.props || []);

    const main = `\tfunction __context(__default = {}) {
${Object.keys(this.snippets).map((_: string) => `const ${_} = $$props.${_} ?? __snippets.${_};`)}
${matched.code}
${this.context === 'client'
    ? `\t\treturn {__default,__scope:{${lets.join(',')}}};`
    : `\t\tconst __callback = () => ({${lets.join(',')}});
\t\treturn {__default,__callback};`}
\t}`;

    let mod = this.module?.code || '';
    this.module?.children.forEach((_: string) => {
      mod = mod.replace(_, _.replace(/\.(?:md|html)/, '.generated.mjs'));
    });

    const js = `/* eslint-disable */${mod}${prelude}
export const __handler = ($$props, __loader${this.context === 'client' ? ', self' : ''}) => {
${main}
${this.context === 'client'
    ? '\treturn {__context};'
    : '\treturn {__context};'}
};

export const __routes = ${JSON.stringify(matched.routes)};
${this.$prefix}
${isGTK ? this.buildGTKTemplate(template, lets) : `export const __vdom = ($$) => [${Block.wrap(template)}];`}
export const __exported = ${JSON.stringify(exported)};
export const __functions = {${calls.join(',')}};
export default {${isGTK ? defaults.replace('__vdom', '__gtk') : defaults},__functions,__exported,__handler,__routes};
for (const [, fn] of Object.entries(__functions)) fn.$ = __src;
`;

    const code = lets.length > 0
      ? js.replace(/\(\$\$\)/g, `($$$$,{${lets.join(',')},...$$$$props})`)
      : js;

    return code;
  }

  buildGTKTemplate(template: string, stateVars: string[] = []): string {
    const subs = getSubscriptions();
    
    const widgetCode = template.trim().startsWith('[')
      ? template.trim().slice(1, -1)
      : template.trim();
    
    const stateParams = stateVars.join(', ');
    let code = `export const __gtk = ({ ${this.opts.gtkImports || 'vstack, hstack, box, label, button, entry, toggle, check, scroll, stack, overlay, paned, expander, revealer, frame, progress, level, spinner, image, dropdown, calendar, listview'} }, { ${stateParams} }) => {\n`;
    
    code += `\tconst self = {};\n`;
    code += `\tconst widget = ${widgetCode};\n`;
    
    if (subs.length > 0) {
      code += `\n\t// Signal subscriptions\n`;
      for (const sub of subs) {
        code += `\t${sub}\n`;
      }
    }
    
    code += `\treturn { widget, self };\n`;
    code += `};\n`;
    
    return code;
  }

  static exports(code: string): string {
    return code
      .replace(/\bexport\s+(let|const)\s+(\w+)\s*(?=[\n;])/g, '$1 $2 = $$$$props.$2')
      .replace(/\bexport\s+(let|const)\s+(\w+)\s*=/g, '$1 $2 = $$$$props.$2 ??')
      .replace(/\bexport\s+function\s+(\w+)\s*\(/g, 'let $1 = $$$$props.$1 ?? function $1(')
      .replace(/\bexport\s+default\b/, '__default =')
      .replace(/\bexport\s*\{([^;]+?)\}/g, (_: string, $1: string) => $1.split(',').map((expr: string) => {
        const [a, b] = expr.trim().split(/\sas\s/);
        return a && b ? `${a} = $$props.${b} ?? ${a};\n` : '';
      }).join(''));
  }

  static module(code: string, routes?: boolean): string {
    const { interlude } = Block.script(code, true);

    let out = interlude;

    if (routes) {
      out = out.replace(RE_MATCH_ROUTES, (_: string, _verb: string, _path: string, alias: string) => _.replace(alias, ignore));
    }

    return out
      .replace(/\bexport\s*\{\s*([^;]+?)\s*\}/g, (_: string, $1: string) => `({${$1.split(' as ').reverse().join(': ')}})`)
      .replace(/\bexport\s+default\b/g, 'const _default=')
      .replace(/\bexport\b/g, ignore);
  }

  static script(code: string, inline?: boolean, basedir: string = Template.shared): { prelude: string; interlude: string; hasImports?: boolean } {
    const internals: string[] = [];

    let lastChunk = '';
    code = code.replace(RE_MATCH_IMPORTS, (_: string, $1: string, _2: string, $3: string) => {
      const name = $1.replace(/[*]\s*as/, ignore).replace(/\sas\s/g, '  : ');
      const symbols = `const ${name}`;

      if (['jamrock', 'jamrock:conn', 'jamrock:hooks', 'jamrock:shared'].includes($3)) {
        internals.push(`${symbols} = __loader('${$3}');`);
        return '';
      }

      if ($3.includes('jamrock:')) {
        return lastChunk = inline
          ? `${symbols} = import('${$3}');`
          : `import ${$1} from '${basedir}/${$3.replace('jamrock:', 'lib/')}';`;
      }

      if ($3[0] === '.' && !($3.includes('.md') || $3.includes('.html'))) {
        return lastChunk = inline
          ? `${symbols} = import('${$3}');`
          : `import ${$1} from /*@@*/__resolve('${$3}');`;
      }

      if ($3.includes('.md') || $3.includes('.html')) {
        const suffix = typeof (globalThis as any).imports === 'undefined'
          ? `?_=${String(Date.now())}`
          : '';

        return lastChunk = inline
          ? `${symbols} = import('${$3}');`
          : _.replace(/\.(?:md|html)/, `.generated.mjs${suffix}`);
      }

      return lastChunk = _;
    });

    let prelude = '';
    let interlude = '';

    if (lastChunk) {
      const offset = code.indexOf(lastChunk) + lastChunk.length;
      const before = code.substr(0, offset);
      const after = code.substr(offset);

      prelude = before;
      interlude = after;
    } else {
      interlude = code;
    }

    interlude = internals.concat(interlude).join('\n').replace(/\n+/g, '\n');

    return { prelude, interlude };
  }

  static unwrap(code: string, source: string, target: string): string {
    const info = [
      source ? `\nexport const __src = '${source}';` : '',
      target ? `\nexport const __dest = '${target}';` : '',
    ].join('');

    const base = source ? Template.dirname(source) : null;
    const leaf = target ? Template.dirname(target) : null;

    if (!code.replace) dump({ code, source });

    return code
      .replace(RE_EXPORT_DEFAULT, (_: string) => [info, _].join('\n'))
      .replace(RE_RESOLVE_IMPORTS, (_: string, src: string) => {
        const a = Template.join(base!, src);
        const b = Template.join(leaf!, src);

        if (a === b) return `'${src}'`;

        const c = Template.relative(b, a);

        return `'${c}'`;
      });
  }

  static wrap(code: string): string {
    return `/*<![CDATA[*/${code}/*]]>*/`;
  }
}

...
function parse(html, file) {
  ...
  try {
    ...
    const tree = walk(himalaya.parse(html, { ...himalaya.parseDefaults, includePositions }));
...
function explode(tpl, find, offset) {
  const chunks = [];
...
function decorate(chunk, locate) {
...
function transform(chunk, isAsync) {
  if (!chunk || typeof chunk !== 'object') return chunk;
  if (Array.isArray(chunk)) return chunk.map(x => transform(x, isAsync));
  if (chunk.mark === '#each') {
    return {
      ...chunk,

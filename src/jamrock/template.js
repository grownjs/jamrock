...
const { createContext } = require('somedom');
const { encode, decode } = require('he');
const reExport = require('rewrite-exports');
const Mortero = require('mortero');

const {
  File, isFile, isSource, isFunction, isGenerator, isIterable, isThenable, hasChanged,
  readFile, writeFile, identifier, relative, realpath, filename, status, trace,
  unlink, exists, mtime, nofile, nodir, noext, rmdir, join, time, use,
  safeJSON, colors, load,
} = require('../util');

const { req } = require('./matcher');
const { renderSync } = require('../render');
const { render, parts, rewrite } = require('../render/transpile');
const { loop, store: Store, compiler } = require('../reactor');

const {
  block, html, util, core,
} = require('../markup');
...
Mortero.use([{
  ...
  run: ({ register }) => {
    register(['some'], (params, done) => {
      render(parts(params.source, params.filepath), result => {
        params._bundle = true;
...
class Template {
  ...
  async resolve(ctx, props, include, callback) {
    ...
    const _locs = this._tpl.locations.reduce((memo, cur) => {
      ...
      cur.locals.forEach(local => {
        if (local.name !== 'this' && !seen.includes(local.name)) {
          seen.push(local.name);
    ...
    if (!this._tpl.prelude) {
      deferred = Template.set(ctx, data, this, _uuid, _locs, _anchor, callback);
    } else {
      ...
      retval = await this._tpl(ctx.$$, props, ctx.loader, loop, ctx.debugger, async payload => {
        try {
    ...
    const [content, metadata, document, attributes] = await Promise.all([
      block.render({ props: _props, _slots: this._slots, render: this._tpl.render }, data, _render),
      block.render({ props: _props, _slots: this._slots, render: this._tpl.metadata }, data),
      block.render({ props: _props, render: this._tpl.document }, data),
      block.render({ props: _props, render: this._tpl.attributes }, data),
    ]);
  ...
  interpolate(kind, props) {
    if (this._parent) {
      this._parent._hooks = this._parent._hooks || this._hooks;
      this._parent._hooks[kind].push(props);
    } else {
      this._hooks[kind].push(props);
    }
  }
  ...
  subscribe(kind, cb) {
    ...
    if (this._parent) {
      this._parent._hooks = this._parent._hooks || this._hooks;
      this._parent._hooks[kind].push(calls);
    } else {
      this._hooks[kind].push(calls);
    }

    return (...args) => {
      const deferred = cb.apply(null, args);
  ...
  dispatch(kind) {
    return Promise.all(this._hooks[kind].map(calls => Promise.all(calls)));
  }
  ...
  invoke(ctx, chunk, payload) {
    const _render = async (_chunk, locals) => {
      ...
      return block.render(_chunk, locals, _render);
    };
  ...
  static set(ctx, data, self, uuid, locals, _anchor, callback) {
    const deferred = [];

    if (!_anchor.includes('#0')) {
      data[util.__ANCHOR__] = _anchor;
    }
  ...
  static require(mod, options) {
    const file = realpath(mod);
    const src = relative(file, true);

    return async (ctx, container) => {
      const tpl = await Template.compile(src, file, options);
      const _template = use(tpl.destination);

      return {
        context: Template.create(ctx, file, { ...options, container }, _template),
        template: Template.from(src, _template, { ...options, container }),
      };
    };
  }

  static create($$, mod, options, _template) {
    const handler = _template || use(mod);
  ...
  static stringify(result, context, options, javascript, stylesheets, inlineScripts) {
    const body = html.taggify(result, context.buffer);
    const head = html.taggify(context.meta, context.buffer);
    ...
    let content = `${prefix}<body${html.attributes(context.attrs)}>\n${body + suffix}</body>`;
    content = `<!DOCTYPE html>\n<html${html.attributes(context.doc)}>\n${content}</html>`;
  ...
  static transform($$, mod, file, main, paths) {
    ...
    } = compiler.transform($$, '$$props', 'await $$loader', false, _ => {
  ...
  static async bundle(name, input, dest, runtime) {
    ...
    writeFile(dest, rewrite(result.source));

    if (!process.silent) {
      status(relative(dest));
    }
  }
  ...
  static async compile(src, file, options = {}) {
    let destFile;
    if (options.dest) {
      destFile = realpath(join(options.dest, src.replace(/\.\w+$/, '.js').replace(/^[^/]+?\//, '')));
    ...
    const markup = options.component || html.parts(readFile(file), file);
    const script = markup.scripts.filter(x => x.isMain).map(x => x.body).join('\n');
    const main = compiler.variables(markup.scripts.filter(x => x.shared).map(x => x.body).join('\n'));
    const cwd = relative(nofile(file), `${process.cwd()}/`, true);
    const $$ = compiler.variables(script, true);
    const re = relative(nofile(file));
    const id = nodir(cwd);
    ...
    if (markup.styles.some(x => x.scoped)) {
      scope = identifier(nodir(file));
    }

    const partial = block.compile(markup.template, file, tree => {
      obj = html.metadata(tree, scope);
    ...
    obj.components.forEach(key => {
      ...
      if (ref.includes(':')) {
        ...
        ref = join(base, dir.join('/'));
        ref = !exists(ref) ? `${ref}.js` : ref;
      } else {
      ...
      const outFile = `${join(options.dest, dirs.slice(len + 1).join('/'))}.js`;
    ...
    obj.attributes = {
      template: `{${partial.enhance(core.Expression.props(obj.markup.attributes || {}, '', 0, true, locate), true)}};`,
    };
    obj.document = {
      template: `{${partial.enhance(core.Expression.props(obj.markup.document || {}, '', 0, true, locate), true)}};`,
    };
    obj.metadata = {
  ...
  static async render(tpl, data, source, extension) {
    ...
    if (typeof tpl === 'object') {
      const result = await new Promise((resolve, reject) => {
        ...
        const partial = Mortero.parse(realpath(filepath), tpl.body, {
          write: false,
          watch: false,
          ...
          aliases: {
            jamrock: join(__dirname, 'browser.js'),
          },
        ...
        partial(data, (err, output) => {
          ...
          if (output.extension === 'css' && tpl.scoped) {
            output.source = html.stylesheet(tpl.scope.join('='), output.source);
          }
          resolve(output);
        });
      });
  ...
  static async loader($$, mod, file, source, options) {
    ...
    if (file === 'jamrock/utilities') {
      return {
        File, rmdir, nodir, noext, nofile, mtime, unlink, exists, encode, decode, colors,
        filename, relative, realpath, identifier, isFile, readFile, writeFile, hasChanged,
      };
    }

    if (file === 'jamrock/components') {
      const exts = ['Link', 'Tabs', 'Route', 'Form', 'Input', 'Failure', 'Debugger'];
      const map = {};

      return exts.reduce((prev, cur) => prev
    ...
    if (file.indexOf('~/') === 0) {
      file = realpath(file.replace('~', '.'));
    }

    const [base, field] = file.split(':');

    try {
      let res = './'.includes(base.charAt()) && Template.locate(nofile(mod), base);

      res = !exists(res)
        ? Template.locate(nofile(source), base)
        : res;
  ...
  static locate(base, target) {
    ...
    for (let i = 0; i < exts.length; i += 1) {
      ...
      if (isFile(b)) return realpath(b);
      if (isFile(c)) return realpath(c);
    }
  }
}

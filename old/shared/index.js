...
export function writeFile(destination, content) {
...
export function readFile(source, check) {
  try {
...
export function relative(file, clean, complete) {
  if (typeof clean === 'string') {
    return complete
      ? path.resolve(file, clean)
      : path.resolve(path.dirname(clean), file);
  }
...
export function hilight(code, lang) {
  ...
  let { value } = hljs.highlight(code, { language: lang });
...
export function stack(tpl, line, column, markup) {
...
export function frame(tpl, src, offset) {
  let line = 0;
  let col = 0;
...
export function trace(e, props, source, include) {
...
export function use(file) {
  return _loader(file).catch(e => {
    e.sample = trace(e, { file }, readFile(file), true);
    throw e;
  });
}

export function load(code, loader) {
  let temp = code;
  let offset = 0;
  let matches;
